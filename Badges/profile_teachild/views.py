...
def profile_view(request):
    ...
    if hasattr(user, 'teacher'):
        ...
        return render(request, 'profile/teacher_profile.html', {'form': form})

    elif hasattr(user, 'childprofile'):
        # Профиль ученика (только просмотр)
        child = user.childprofile
        return render(request, 'profile/child_profile.html', {'child': child})
...
def edit_child_profile(request, child_id):
    ...
    if not (user.is_staff or (hasattr(user, 'teacher') and child.teacher == user.teacher)):
        return render(request, 'profile/access_denied.html')
    ...
    return render(request, 'profile/edit_child_profile.html', {'form': form})